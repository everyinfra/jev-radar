...
### 速率与并发上限(jev-1.13,官方 Models 页)

| 限制项 | 数值 |
|---|---|
| 请求频率 | **1,200 请求/分钟**(即持续 20 req/s) |
| 吞吐量 | **250,000 token/秒** |
| 单请求上下文 | 64k token(state+全部问题);单问题上限 32k(state+最长问题) |

超出任一限制返回 429(带 retry-after),官方 SDK 自动指数退避;限制"可能不经通知调整",更高配额走企业版(sales@typesafe.ai)。轻量状态(几百~几千 token)下瓶颈通常是 1,200 RPM;重状态(万级 token)下 250k tok/s 约等于每秒 25 个大请求。
